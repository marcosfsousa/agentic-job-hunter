...
#
# Copied from github.com/marcosfsousa/gh-repo-baseline (templates/tests/), which
...
#
# Copied at upstream ae78c6b3 (2026-07-31). Diff this file against that
...
# rather than eyeballing 1,300 lines.
#
...
# tests.yml, not ci.yml — this repo's deterministic gate. The other two
# workflows are deliberately not covered here and must never become required
# checks: daily_run.yml is `schedule` + `workflow_dispatch` and never reports on
# a pull request at all, and claude.yml carries a `paths-ignore`, so a prose-only
# pull request does not trigger it. Requiring either would leave a pull request
# waiting forever on a check that never arrives — the exact failure this file
# exists to make loud, introduced by the fix for it.
...
class TestGuardIsNotVacuous:
    ...
    def test_step_names_are_not_collected(self):
        ...
        # Per-repo, like the constants above: this is `name: Tests` in tests.yml.
        assert "Tests" not in collected
